...
def calculate_wallstreet_fair_range(ticker_symbol, info, current_price, shiller_pe=31.5):
    ...
    try:
        ...
        if factors:
            ...
            return mid_fv * 0.9, mid_fv * 1.1 # 輸出上下 10% 區間
            
    except Exception:
        pass
    return current_price * 0.9, current_price * 1.02
...
def generate_quant_signals(df_data, atr_mult, rsi_val, drop_pct, bias_val, use_market_fil, ticker_symbol):
    ...
    low_absorb_bound = df['MA20_actual'] - (df['ATR'] * float(atr_mult))
    price_cond = (df['Low'] <= low_absorb_bound) | (df['Low'] <= df['BB_Lower'])
    rsi_cond = df['RSI'] <= float(rsi_val)
    ...
    for date, is_triggered in price_cond.items():
        ...
        if is_triggered and rsi_cond.loc[date]:
            ...
            is_allowed = is_market_safe_today or (df.loc[date, 'MA20_actual'] >= current_ma200) or (current_ma200_bias >= float(bias_val))
            if use_market_fil and not is_market_safe_today and (current_ma200_bias < float(bias_val)): is_allowed = False
            ...
            if current_yw in served_weeks:
                price_drop_target = last_buy_price * (1 - (float(drop_pct) / 100))
...
if action_alerts:
    ...
    st.dataframe(df_promo.sort_values(by="代碼"), use_container_width=True, hide_index=True)
else:
    st.info("🧘 今日觀察名單內皆無強買標的。")
...
if summary_data:
    ...
    st.dataframe(summary_df.sort_values(by="代碼"), use_container_width=True, hide_index=True)
