...
with macro_col3:
    ...
    calendar_data = {
        ...
        "即時進度與數據結論 / 市場預期": [
            ...
            "🔮 重大焦點：將釋放 2026 下伴年降息終點利率的最新政策密碼。",
...
# 由於需要抓取近 20 日最高價，確保回溯時間足夠（維持 365 天回溯不變）
    ...
    for ticker in active_tickers:
        try:
            ...
            if eps and growth and eps > 0 and growth > 0:
                ...
                vals.append(eps * growth_rate * 1.0) 
            ...
            if vals:
                ...
                if min_fv == max_fv:
                    fair_value_str = f"{currency_symbol}{min_fv:.1f}"
                else:
                    fair_value_str = f"{currency_symbol}{min_fv:.1f} ~ {max_fv:.1f}"
            ...
            if current_price >= latest['MA20'] and latest['MA20'] >= latest['MA200']:
                ...
                if current_price <= low_absorb_price:
                    final_action = "🔥 強力買入"
                    reason_str = f"跌破 MA20 對稱網格下限 (-{atr_multiplier}x ATR)，黃金埋伏機會！"
                elif abs(current_price - latest['MA20'])/latest['MA20'] <= 0.02:
                    final_action = "🟢 買入"
                    reason_str = "拉回到關鍵 MA20 決策線重要支撐區，符合穩定建倉邏輯。"
                elif current_price >= high_toss_price or current_price >= latest['BB_Upper']:
                    final_action = "🔴 賣出"
                    reason_str = f"短線噴發過熱，衝破 MA20 網格上限 (+{atr_multiplier}x ATR)，波段高拋。"
                else:
                    final_action = "⚪ 觀望"
                ...
                if yesterday_close >= latest['MA20'] and current_price < latest['MA20']:
                    final_action = "🚨 強力賣出"
                    reason_str = "剛破 MA20 決策線，趨勢轉空，請果斷離場，拒絕接飛刀。"
                elif current_price >= high_toss_price:
                    final_action = "🔴 賣出"
                    reason_str = f"空頭反彈觸及 MA20 網格上限 (+{atr_multiplier}x ATR)，逃命高拋點。"
                elif current_price <= low_absorb_price:
                    final_action = "🟢 買入"
                    reason_str = f"空頭下跌破 MA20 網格下限 (-{atr_multiplier}x ATR)，極小倉位試探反彈。"
                else:
                    final_action = "⚪ 觀望"
                    reason_str = "空頭結構中，堅決保持空倉觀望。"
                    
            else:
                market_state = "↕️ 箱型震盪 (會震盪)"
                if current_price <= low_absorb_price * 1.005 and current_price >= low_absorb_price * 0.995:
                    final_action = "🔥 強力買入"
                    reason_str = f"觸及 MA20 網格下限 (-{atr_multiplier}x ATR)，低吸買入。"
                elif current_price >= high_toss_price * 0.995 and current_price <= high_toss_price * 1.005:
                    final_action = "🚨 強力賣出"
                    reason_str = f"觸及 MA20 網格上限 (+{atr_multiplier}x ATR)，高拋賣出。"
                else:
                    final_action = "⚪ 觀望"
            ...
            if final_action != "⚪ 觀望":
                action_alerts.append({
                    ...
                    "移動停利價位": trailing_stop_str,  # 警示面板同步新增
                    "昨日收盤價": f"{currency_symbol}{yesterday_close:.1f}",
            ...
            summary_data.append({
                ...
                "當前股價": f"{currency_symbol}{current_price:.1f}",
                "公允價值區間": fair_value_str,
                "移動停利價位": trailing_stop_str,  # 大看板表格新增欄位
                "昨收盤價": f"{currency_symbol}{yesterday_close:.1f}",
                "MA20": f"{currency_symbol}{ma20_center:.1f}",
                "市場狀態": market_state,
                "綜合建議": final_action,
                "買點": f"{currency_symbol}{low_absorb_price:.1f}",
                "賣點": f"{currency_symbol}{high_toss_price:.1f}",
                "精簡決策原因": reason_str
            })
        except Exception:
            pass
    ...
    try:
        ...
        if not df_detail.empty and len(df_detail) > 200:
            ...
            if info:
                ...
                col_f1.metric("公司淨利率", f"{net_margin * 100:.1f}%")
                col_f2.metric("流動比率", f"{current_ratio:.1f}", "✅ 安全 (>1.5)" if current_ratio > 1.5 else "⚠️ 偏低")
