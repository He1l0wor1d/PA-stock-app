...
if selected_strategy and selected_strategy != st.session_state.strategy_selection:
    ...
    if selected_strategy == "🛡️ 保守型 (抄底)":
        ...
        st.session_state.p_rsi = 29          # 完美釋放 TSM 2025/4 訊號
...
if st.session_state.strategy_selection == "🛡️ 保守型 (抄底)":
    st.success(f"### 當前運行策略：{st.session_state.strategy_selection}\n*特點：【💥右側動能急跌鎖與主線濾網雙重啟動】底層已徹底封殺 SOFI 與 NOW(剛破位高檔暴雷股)。完美放行 TSM / NVDA 2025/04 世紀黃金底！*")
...
with st.spinner("正在提煉核心決策..."):
    for ticker in active_tickers:
        try:
            ...
            # 🛠️ 核心進化防線：計算過去 5 天的最高收盤價，用來捕捉 NOW 這種高位剛崩塌、動能失控的毒蘋果
            df['Max_Close_5D'] = df['Close'].rolling(window=5).max()
            df['Crash_Pct_5D'] = ((df['Max_Close_5D'] - df['Close']) / df['Max_Close_5D']) * 100
            ...
            for date, is_triggered in price_cond.items():
                if is_triggered and rsi_cond.loc[date]:
                    ...
                    # 🛠️ 核心修正：保守型抄底雙層濾網
                    if st.session_state.strategy_selection == "🛡️ 保守型 (抄底)":
                        # 1. 產業降維打擊：只留四大硬核主線，直接永久抹除 SOFI 的飛刀
                        allowed_core_sectors = ["晶圓代工製程", "AI晶片與設計", "AI巨頭與軟體", "市值型大盤"]
                        if ticker_sector not in allowed_core_sectors:
                            is_allowed = False
                        
                        # 2. 右側動能急跌鎖：如果 5 天內收盤價砸盤跌幅超過 15%，代表這是高位剛暴雷的假黃金坑，強制封鎖！直接秒殺 NOW
                        if df.loc[date, 'Crash_Pct_5D'] > 15.0:
                            is_allowed = False
...
if selected_stock:
    try:
        ...
        if not df_detail.empty and len(df_detail) > 240:
            ...
            df_detail['Max_Close_5D'] = df_detail['Close'].rolling(window=5).max()
            df_detail['Crash_Pct_5D'] = ((df_detail['Max_Close_5D'] - df_detail['Close']) / df_detail['Max_Close_5D']) * 100
            ...
            for date, is_triggered in price_cond.items():
                if is_triggered and rsi_cond.loc[date]:
                    ...
                    if st.session_state.strategy_selection == "🛡️ 保守型 (抄底)":
                        ...
                        if ticker_sector not in allowed_core_sectors:
                            is_allowed = False
                        if df_detail.loc[date, 'Crash_Pct_5D'] > 15.0:
                            is_allowed = False
                    ...
                    if last_date_det is None:
                        ...
                        if is_extreme_crash_det or days_passed > wave_window_days:
                            sparse_buy_det[date] = True
                            last_date_det = date
                            last_price_det = current_touch_price_det
                        else:
...
with st.spinner("正在進行時光回溯與策略模擬建倉..."):
    ...
    for ticker in active_tickers:
        try:
            ...
            df_bt['Max_Close_5D'] = df_bt['Close'].rolling(window=5).max()
            df_bt['Crash_Pct_5D'] = ((df_bt['Max_Close_5D'] - df_bt['Close']) / df_bt['Max_Close_5D']) * 100
