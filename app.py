...
rsi_filter_val = st.sidebar.slider("RSI 超賣過濾限制 (低於此值才觸發強買) [💡建議調低至25以過濾初跌]", 15, 45, 32, 1)
    ...
    for ticker in active_tickers:
        try:
            ...
            if use_market_filter and not is_market_safe:
                is_extreme_panic = False # 大盤崩盤時，主動鎖定保護，拒絕接個股飛刀
            ...
            # 🛠️ 修正需求 (1)：只保留 '🔥 強力買入' 與 '🟢 買入'，移除不相干欄位，新增買點
...
if selected_stock:
    try:
        ...
        if not df_detail.empty and len(df_detail) > 200:
            ...
            # 🛠️ 修正需求 (1) & (2)：徹底排除大盤限制干擾以捕捉歷史極端坑。移除正三角形，只保留火焰文字標記
            ...
            # 🛠️ 修正需求 (1)：移除正三角形標記，極致精簡改用『純粹火焰圖案文字型態』
            if not buy_signals.empty:
                fig.add_trace(go.Scatter(
                    x=buy_signals.index,
                    y=buy_signals['Low'] * 0.95,  
                    mode='text',
                    text=[f"🔥強烈買入<br>{p:.1f}" for p in buy_signals['Close']],
                    textposition="bottom center",
                    textfont=dict(color="darkgoldenrod", size=11, family="Arial Black"),
                    name='🔥 強力買入點'
                ))
...
with backtest_col1:
    # 🛠️ 修正需求 (3)：回測日期預設修改為 2026 年 5 月 1 日
        ...
        try:
            ...
            for date, row in df_scan.iterrows():
                ...
                if use_market_filter and not is_market_safe_past:
                    is_past_strong_buy = False # 回測中也啟動風控
...
if backtest_results:
    ...
    # 🛠️ 修正需求 (2)：同步將 S&P 500 的報酬表現並列展示在面板上
