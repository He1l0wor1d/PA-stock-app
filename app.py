...
if selected_strategy and selected_strategy != st.session_state.strategy_selection:
    ...
    if selected_strategy == "🛡️ 保守型 (抄底)":
        st.session_state.p_atr = 1.8         # 退回黃金平衡點
        st.session_state.p_rsi = 29          # 確保 TSM 2025/4 完美開槍
        st.session_state.p_window = 45       # 空間排他視窗
...
if st.session_state.strategy_selection == "🛡️ 保守型 (抄底)":
    st.success(f"### 當前運行策略：{st.session_state.strategy_selection}\n*特點：【⚔️ 空間排他至高無上鎖 + 5日鋼鐵冷卻線】退回黃金抄底版本，確保 TSM 2025/04 順利開槍、SOFI 物理封殺，且任何策略信號間隔小於5天一律強制阻斷！*")
        ...
        try:
            ...
            for date, is_triggered in price_cond.items():
                if is_triggered and rsi_cond.loc[date]:
                    ...
                    # 🛡️ 退回黃金防禦：只過濾 SOFI 的產業濾網與前日多頭排列鎖
                    if st.session_state.strategy_selection == "🛡️ 保守型 (抄底)":
                    ...
                    if last_buy_date is None:
                        ...
                        # 🛠️ 鋼鐵策略升級 1：空間排他鎖至高無上順位！小於滑桿設定天數一律直接阻斷
                        if days_passed <= wave_window_days:
                            # 🛠️ 鋼鐵策略升級 2：除非「再度暴跌」超越上一次買點的再跌幅門檻，才允許提早打破排他鎖
                            price_drop_target = last_buy_price * (1 - (min_drop_pct / 100))
                            if days_passed >= 5 and current_touch_price <= price_drop_target:
                                # 滿足更便宜的跌幅門檻，且至少過了 5 天，允許放行補槍
                                sparse_strong_buy[date] = True
                                last_buy_date = date
                                last_buy_price = current_touch_price
                            continue
                        
                        # 超過排他天數後，滿足修正A（踩踏）或正常分批條件放行
            ...
            for date, is_triggered in price_cond.items():
                if is_triggered and rsi_cond.loc[date]:
                    ...
                    if last_date_det is None:
                        ...
                        # 圖表同步最高順位排他鎖
                        if days_passed <= wave_window_days:
                            price_drop_target_det = last_price_det * (1 - (min_drop_pct / 100))
                            if days_passed >= 5 and current_touch_price_det <= price_drop_target_det:
                                sparse_buy_det[date] = True
                                last_date_det = date
                                last_price_det = current_touch_price_det
                            continue
                ...
                if is_raw_strong_buy:
                    ...
                    if last_bt_date is None:
                        ...
                        if days_passed <= wave_window_days:
                            ...
                            if days_passed >= 5 and bt_touch_price <= price_drop_target_bt:
                                is_past_strong_buy = True
                                total_strong_buy_count += 1
                                last_bt_date = date
                                last_bt_price = bt_touch_price
