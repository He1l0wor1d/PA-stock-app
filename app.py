...
# 🛠️ 視覺增強：追蹤目前被選擇的策略標籤
if "current_strategy_name" not in st.session_state: st.session_state.current_strategy_name = "🎛️ 自訂微調模式"

btn_col1, btn_col2, btn_col3 = st.sidebar.columns(3)

with btn_col1:
    if st.button("🛡️ 保守型\n(大跌抄底)"):
        st.session_state.p_atr = 2.2         
        st.session_state.p_rsi = 24          
        st.session_state.p_window = 45       
        st.session_state.p_drop = 8          
        st.session_state.p_bias = 7          
        st.session_state.current_strategy_name = "🛡️ 保守型 (大跌抄底模式)"
        st.rerun()
...
with btn_col2:
    if st.button("💎 中等型\n(價值低估)"):
        ...
        st.session_state.current_strategy_name = "💎 中等型 (板塊價值模式)"
        st.rerun()

with btn_col3:
    if st.button("⚡ 積極型\n(高頻網格)"):
        st.session_state.p_atr = 0.7         
        st.session_state.p_rsi = 45          
        st.session_state.p_window = 10       
        st.session_state.p_drop = 2          
        st.session_state.p_bias = 2          
        st.session_state.current_strategy_name = "⚡ 積極型 (高頻動能網格) — 💡 警報：此模式資金消耗極大！"
        st.rerun()

st.sidebar.header("📊 對稱網格參數微調")
atr_period = 14

# 監聽手動滑桿調整，如果使用者動了滑桿，自動切換回自訂模式
...
# 如果目前參數跟 session 記錄的不對等，判定使用者手動調整了
if (atr_multiplier != st.session_state.p_atr or rsi_filter_val != st.session_state.p_rsi or 
    ...
    st.session_state.current_strategy_name = "🎛️ 自訂微調模式"

use_market_filter = st.sidebar.checkbox("啟用大盤多空防護鎖 (S&P500破年線時全面暫停強買)", value=True)

# 🛠️ 視覺升級核心：主面板頂部噴射醒目的策略狀態框
if "🛡️" in st.session_state.current_strategy_name:
    st.success(f"### 當前運行策略：{st.session_state.current_strategy_name}\n*特點：出手頻率極低，主打 2025/4 這種史詩級非理性崩盤，資金利用安全率極高。*")
elif "💎" in st.session_state.current_strategy_name:
    st.info(f"### 當前運行策略：{st.session_state.current_strategy_name}\n*特點：出手頻率中等，捕捉板塊價值輪動拉回，用多標的分散布局換取中上回報。*")
elif "⚡" in st.session_state.current_strategy_name:
    st.warning(f"### 當前運行策略：{st.session_state.current_strategy_name}\n*特點：出手高頻且極其敏感，容易在半山腰反覆吃單被套。**實戰操作需具備極其雄厚的無限資金支持！***")
else:
    st.markdown(f"##### ⚖️ 當前運行策略：`{st.session_state.current_strategy_name}` (滑桿自訂參數狀態中)")
