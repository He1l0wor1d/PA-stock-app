...
with macro_col3:
    ...
    calendar_data = {
        ...
        "關鍵數據 / 財經大事": ["紐約聯儲製造業指數", "RBA 貨政策紀要", "EIA 原油庫存", "Fed 貨幣政策紀要", "美國 4 月核心 PCE"],
    ...
    for ticker in active_tickers:
        try:
            ...
            if ma20_center >= latest_ma200:
                ...
                if yesterday_close >= ma20_center parks current_price < ma20_center: 
                    final_action = "🚨 強力賣出"
...
if selected_stock:
    try:
        ...
        if not df_detail.empty and len(df_detail) > 200:
            ...
            # 🛡️ 核心修正：將 AI 撈取邏輯放在獨立的內部 try-except 中，防範因其他底層庫出錯引起的阻斷
            try:
            ...
            # 🛡️ 終極防禦層：將 yfinance 的 calendar 單獨進行最嚴格的安全包裹，防止 yfinance 底層庫變更阻斷主程式
            try:
                # 避開 yfinance 直接呼叫 .calendar 觸發的 Unknown field 內部解析 bug，改用安全性更佳的屬性檢查
