...
with st.spinner("正在提煉五等核心 ACTION 決策與計算三大模型公允價值..."):
    for ticker in active_tickers:
        try:
            ...
            # ==========================================
            # 🧠 核心降維邏輯：對齊五等決策紅綠燈 (股價輸出四捨五入小數點第一位)
            # ==========================================
            ...
            if current_price >= latest['MA20'] and latest['MA20'] >= latest['MA200']:
                ...
                if current_price <= low_absorb_price:
                    ...
                    final_action = "⚪ 觀望"
                    reason_str = f"多頭結構健全，已為您計算最新 MA20 低吸位 {currency_symbol}{low_absorb_price:.1f}，未到請安心持股。"
            ...
            # 軌道三：趨勢會震盪 (箱型橫盤)
                ...
                if current_price <= low_absorb_price * 1.005 and current_price >= low_absorb_price * 0.995:
                    ...
                    final_action = "⚪ 觀望"
                    reason_str = f"處於箱型震盪中樞。最新 MA20 對稱網格：低吸買點 {currency_symbol}{low_absorb_price:.1f} | 高拋賣點 {currency_symbol}{high_toss_price:.1f}。"

            # 新增欄位皆已在背景處理完成並格式化至小數點後第一位
            if final_action != "⚪ 觀望":
                action_alerts.append({
                    ...
                    "20日均線(MA20)": f"{currency_symbol}{ma20_center:.1f}",
                    "精簡決策原因": reason_str
                })
...
# ✨ 新增功能：投資決策參考守則折疊面板 (原生安插在大看板正下方)
with st.expander("💡 投資決策參考守則：如何正確對照公允價值？"):
    st.markdown("""
    * **情境 A (網格買點 + 遠低於公允價值)：** **大膽買進**。技術面與基本面形成共振，擁有極高的「安全邊際」。
    * **情境 B (網格買點 + 公允價值接近或低於市價)：** **縮小部位或放棄**。基本面不具吸引力，高機率是接飛刀的陷阱，請把資金留給情境 A 的股票。
    * **情境 C (網格賣點 + 遠低於公允價值)：** **分批賣出，保留底倉**。基本面依然強勁，不一定要清倉，可留部分底倉參與價值回歸。
    * **情境 D (網格賣點 + 公允價值低於市價)：** **果斷清倉高拋**。典型的基本面與技術面雙雙泡沫超漲，建議全數離場守住獲利。
    """)
