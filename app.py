...
# ==============================================================================
# 3. 內建核心產業與供應鏈地圖 (嚴格遵守 10 字以內，無英文無台股)
# ==============================================================================
INITIAL_SECTOR_MAP = {
...
with st.spinner("正在提煉五等核心 ACTION 決策與計算三大模型公允價值..."):
    for ticker in active_tickers:
        try:
            ...
            if eps and hist_pe and eps > 0 and hist_pe > 0: vals.append(eps * hist_pe)
            growth = info.get('earningsGrowth')
            if eps and growth and eps > 0 and growth > 0: vals.append(eps * (growth * 100) * 1.0) 
            dps, div_yield = info.get('dividendRate'), info.get('dividendYield')
            ...
            if vals:
                if len(vals) == 3:
                    vals.sort()
                    median = vals[1]
                    filtered = [v for v in vals if abs(v - median) / median <= 0.5] if median > 0 else vals
                    if not filtered: filtered = vals  
                else: filtered = vals
                min_fv, max_fv = min(filtered), max(filtered)
            ...
            # ==========================================
            # ✨ 邏輯重磅修復：以「均線排列」判定多空，解決無法觸發買點的矛盾
            # ==========================================
            market_state = "⚪ 觀望"
            final_action = "⚪ 觀望"
            ...
            try:
                ...
                if news_list and isinstance(news_list, list) and len(news_list) > 0:
                    ...
                    for article in news_list[:3]:
                        title, link, publisher = article.get('title', '無標題'), article.get('link', '#'), article.get('publisher', '未知')
                        st.markdown(f"**[{title}]({link})** (來源: {publisher})")
                else: st.info("📰 暫無最新市場消息")
            except Exception: st.info("📰 新聞模組抓取受限")
    except Exception as e: st.error(f"分析載入失敗: {e}")

# ==============================================================================
# ⏳ 策略回測績效驗證 (全新 Scan-Forward 尋找首個買點機制)
# ==============================================================================
st.markdown("---")
...
with backtest_col:
    default_date = datetime.now().date() - timedelta(days=60) # 預設回溯兩個月
...
if backtest_results:
    ...
    st.info(f"自 {bt_date_str} 起算，觀察名單內無任何標的觸發買入條件 (股價未回檔至 20MA 或網格下限)。這顯示目前市場可能處於無腦噴發或空頭，請嘗試將日期往前推！")
