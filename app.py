...
# 🛠️ 修正 (2)：當滑桿參數與當前策略不同時，按鈕立刻切換並高亮標註為「自訂微調」
        ...
        try:
            ...
            for date, is_triggered in price_cond.items():
                if is_triggered and rsi_cond.loc[date]:
                    ...
                    # 🛠️ 修正 (1)：移除所有 hardcoded 的產業與策略名稱限制！全資產遵循相同的牛市前置濾網
                ...
                if is_triggered and rsi_cond.loc[date]:
                    ...
                    if last_det_yw is None:
                        ...
                        if det_yw == last_det_yw:
                            ...
                            if current_touch_price_det <= price_drop_target_det:
                                sparse_buy_det[date] = True
                                last_det_price = current_touch_price_det
                            continue
...
backtest_results = []
portfolio_total_buy_signals = 0 # 🛠️ 累計整個投資組合的強買總開槍次數
...
with st.spinner("正在進行時光回溯與策略模擬建倉..."):
    ...
    for ticker in active_tickers:
        try:
            ...
            df_scan_triggered = pd.Series(0, index=df_scan.index) 
            ...
            for date, row in df_scan.iterrows():
                ...
                is_allowed_bt = is_market_safe_past or (past_ma20 >= past_ma200) or (past_ma200 - past_low)/past_ma200 * 100 >= extreme_ma200_bias
                if use_market_filter and not is_market_safe_past and not (past_ma200 - past_low)/past_ma200 * 100 >= extreme_ma200_bias:
                    is_allowed_bt = False
                ...
                if is_raw_strong_buy and is_allowed_bt:
                    if last_bt_yw is None:
                        ...
                        if bt_yw == last_bt_yw:
                            ...
                            if bt_touch_price <= price_drop_target_bt:
                                df_scan_triggered[date] = 2
                                df_scan_prices[date] = bt_touch_price
                                last_bt_price = bt_touch_price
                            continue
                ...
                elif abs(past_low - past_ma20) / past_ma20 <= 0.02:
                    if last_bt_yw is None:
            ...
            portfolio_total_buy_signals += total_strong_buy_count # 累加器
...
if backtest_results:
    ...
    # 🛠/修正 (4)：看板尾部新增並回報全體總進場開槍次數統計卡片
    st.info(f"📈 策略平均報酬率：**{avg_return:.1f}%** | 🎯 策略勝率：**{win_rate:.1f}%** | 💰 區間內總買入次數：**{portfolio_total_buy_signals} 次** | 📊 同期對比 SPY：**{spy_performance_pct:.1f}%**")
else:
