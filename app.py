...
# ==============================================================================
# 3. 內建核心產業與供應鏈地圖 (✨ 重新精煉分類：不超10字、不含英文字、不強調ETF與台股)
# ==============================================================================
INITIAL_SECTOR_MAP = {
    # 核心基本觀察
    "2330.TW": "半導體晶圓代工",
        ...
        try:
            ...
            if current_price >= latest['MA20'] and latest['MA20'] >= latest['MA200']:
                ...
                if current_price <= low_absorb_price: 
                    final_action = "🔥 強力買入"
                    reason_str = f"跌破 MA20 對稱網格下限 (-{atr_multiplier:.1f}x ATR)，黃金埋伏機會！"
                    ...
                    final_action = "🔴 賣出"
                    reason_str = f"短線噴發過熱，衝破 MA20 網格上限 (+{atr_multiplier:.1f}x ATR)，波段高拋。"
                else:
                    final_action = "⚪ 觀望"
                ...
                if yesterday_close >= latest['MA20'] and current_price < latest['MA20']: 
                    ...
                    final_action = "🔴 賣出"
                    reason_str = f"空頭反彈觸及 MA20 網格上限 (+{atr_multiplier:.1f}x ATR)，逃命高拋點。"
                elif current_price <= low_absorb_price: 
                    final_action = "🟢 買入"
                    reason_str = f"空頭下跌破 MA20 網格下限 (-{atr_multiplier:.1f}x ATR)，極小倉位試探反彈。"
                else:
                    final_action = "⚪ 觀望"
                ...
                if current_price <= low_absorb_price * 1.005 and current_price >= low_absorb_price * 0.995: 
                    final_action = "🔥 強力買入"
                    reason_str = f"精準觸及 MA20 網格下限 (-{atr_multiplier:.1f}x ATR)，網格低吸。"
                elif current_price >= high_toss_price * 0.995 and current_price <= high_toss_price * 1.005: 
                    final_action = "🚨 強力賣出"
                    reason_str = f"精準觸及 MA20 網格上限 (+{atr_multiplier:.1f}x ATR)，網格高拋。"
                else:
                    final_action = "⚪ 觀望"
...
if selected_stock:
    try:
        ...
        if not df_detail.empty and len(df_detail) > 200:
            ...
            # ==============================================================================
            # ✨ 基本面欄位升級：強化核心防空鎖機制 (營收年增率、季度 Capex、PE)
            # ==============================================================================
            # 1. 營收年增率 YoY
            rev_growth = info.get('revenueGrowth')
            rev_growth_str = f"{rev_growth * 100:.1f}%" if rev_growth is not None else "無數據"
                
            # 2. 季度資本支出 Capex (安全校驗機制：防止部分標的無季度表或欄位缺失)
            capex_str = "無數據"
            try:
                quarterly_cashflow = stock_detail.quarterly_cashflow
                if quarterly_cashflow is not None and not quarterly_cashflow.empty:
                    # 先在 index 中做模糊比對，避免大小寫不對稱崩潰
                    matching_keys = [k for k in quarterly_cashflow.index if 'Capital Expenditures' in str(k)]
                    if matching_keys:
                        latest_capex = quarterly_cashflow.loc[matching_keys[0]].dropna().iloc[0]
                        capex_str = f"{abs(latest_capex) / 1000000:.1f} 百萬"
            except Exception:
                pass # 遇到無 Capex 數據的傳產、金融或 ETF 時，直接保留 "無數據" 絕不中斷網頁運作
            ...
            col_f2.metric("最新季度資本支出 (Capex)", capex_str, help="反映企業對 AI、算力集群與基礎設施的投入力道")
