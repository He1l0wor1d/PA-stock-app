...
with macro_col3:
    ...
    calendar_data = {
        ...
        "關鍵數據 / 財經大事": ["紐約聯儲製造業指數", "RBA 貨鏡政策紀要", "EIA 原油庫存", "Fed 貨幣政策紀要", "美國 4 月核心 PCE"],
...
if selected_strategy and selected_strategy != st.session_state.strategy_selection:
    ...
    if selected_strategy == "🛡️ 保守型 (抄底)":
        ...
        st.session_state.p_window = 45       # 空間排他高居最高順位
...
if st.session_state.strategy_selection == "🛡️ 保守型 (抄底)":
    st.success(f"### 當前運行策略：{st.session_state.strategy_selection}\n*特點：【🔥 均線多頭排列排列鎖核心啟動】200MA年線斜率與50MA季線雙向確認，完美將 NOW 這種高位剛轉中線空頭的暴雷股連根拔除！*")
...
with st.spinner("正在提煉核心決策..."):
    for ticker in active_tickers:
        try:
            ...
            df['MA50'] = df['Close'].rolling(window=50).mean()  # 🎯 新增季線
            df['MA200'] = df['Close'].rolling(window=200).mean()
            df['STD20'] = df['Close'].rolling(window=20).std()
            df['BB_Lower'] = df['MA20_actual'] - (2 * df['STD20'])
            df['RSI'] = calculate_rsi(df['Close'], 14)
            
            # 🛠️ 鋼鐵陣線升級：計算年線的長線斜率，以及均線多頭排列狀態
            df['MA200_slope_safe'] = df['MA200'] >= df['MA200'].shift(20)  # 20天內均線要往上走
            df['Ribbon_Bull_Safe'] = df['MA50'] >= df['MA200']             # 50MA必須在200MA上方，杜絕高檔死亡交叉股
            ...
            for date, is_triggered in price_cond.items():
                if is_triggered and rsi_cond.loc[date]:
                    ...
                    if st.session_state.strategy_selection == "🛡️ 保守型 (抄底)":
                        ...
                        # 2. 均線多頭排列死鎖鎖：年線斜率必須往上，且50MA必須在200MA上方！直接封殺 NOW 這種在死叉邊緣的高位崩塌股
                        if not df.loc[date, 'MA200_slope_safe'] or not df.loc[date, 'Ribbon_Bull_Safe']:
                            is_allowed = False
                    ...
                    if last_buy_date is None:
                        ...
                        # 🛠️ 空間排他鎖至高無上順位：只要小於排他時間，任何條件直接作廢不准重複叫單
...
if selected_stock:
    try:
        ...
        if not df_detail.empty and len(df_detail) > 240:
            ...
            df_detail['MA50'] = df_detail['Close'].rolling(window=50).mean()
            ...
            df_detail['MA200_slope_safe'] = df_detail['MA200'] >= df_detail['MA200'].shift(20)
            df_detail['Ribbon_Bull_Safe'] = df_detail['MA50'] >= df_detail['MA200']
            ...
            for date, is_triggered in price_cond.items():
                if is_triggered and rsi_cond.loc[date]:
                    ...
                    if st.session_state.strategy_selection == "🛡️ 保守型 (抄底)":
                        ...
                        if ticker_sector not in allowed_core_sectors:
                            is_allowed = False
                        if not df_detail.loc[date, 'MA200_slope_safe'] or not df_detail.loc[date, 'Ribbon_Bull_Safe']:
                            is_allowed = False
...
with st.spinner("正在進行時光回溯與策略模擬建倉..."):
    ...
    for ticker in active_tickers:
        try:
            ...
            df_bt['MA200_slope_safe'] = df_bt['MA200'] >= df_bt['MA200'].shift(20)
            df_bt['Ribbon_Bull_Safe'] = df_bt['MA50'] >= df_bt['MA200']
            ...
            for date, row in df_scan.iterrows():
                ...
                if is_raw_strong_buy:
                    ...
                    if st.session_state.strategy_selection == "🛡️ 保守型 (抄底)":
                        ...
                        if ticker_sector not in allowed_core_sectors:
                            is_allowed_bt = False
                        if not row['MA200_slope_safe'] or not row['Ribbon_Bull_Safe']:
                            is_allowed_bt = False
